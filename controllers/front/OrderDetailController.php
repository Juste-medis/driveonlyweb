    ...
    public function initContent()
    {
        ...
        if (!$id_order) {
            ...
            if (Validate::isLoadedObject($order) && $order->id_customer == $this->context->customer->id) {
                ...
                $ticket = Db::getInstance()->getRow("SELECT id_message FROM `ps_ets_livechat_ticket_form_message` WHERE `id_order`=$id_order");

                $id_ticket = 0;
                if(isset($ticket['id_message']))
                {
                    $id_ticket = $ticket['id_message'];
                }
