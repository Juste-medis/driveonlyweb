    ...
    {if $nodes|count}
      <ul class="category-sub-menu">
        {foreach from=$nodes item=node}
          <li data-depth="{$depth}">
            {if $depth===0}
              <a href="{$node.link}"{if $node.link == $urls.current_url} class="current"{/if}>{$node.name}</a>
              {if $node.children}
              ...
              {/if}
            {else}
              <a class="category-sub-link" href="{$node.link}"{if $node.link == $urls.current_url} class="current"{/if}>{$node.name}</a>
              {if $node.children}
