...
{if $products}
	...
	{foreach from=$products item=product name=i}
		<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
			...
			<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product}', {$product.id_product_attribute}, '0');" title="{l s='remove this product from my wishlist' d='Modules.Blockwishlist.Shop'}" rel="nofollow">{l s='Delete' d='Modules.Blockwishlist.Shop'}</a>
		</dt>
		...
		<dd class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}" style="font-style:italic;margin:0 0 0 10px;">
			<a href="{$link->getProductLink($product.id_product, $product.link_rewrite)|escape:'html'}" title="{l s='Product detail' d='Modules.Blockwishlist.Shop'}">{$product.attributes_small|escape:'html':'UTF-8'}</a>
		</dd>
		{/if}
	{/foreach}
	</dl>
{else}
	...
	{if isset($error) && $error}
		<dt>{l s='You must create a wishlist before adding products' d='Modules.Blockwishlist.Shop'}</dt>
	{else}
