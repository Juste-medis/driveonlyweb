...
<div class="{if $carousel_tabs == 'true'}in-tabs tab-pane fade{else}none-in-tabs{/if}{if $carousel_active  == 'true' && $carousel_arrows == 'true'} nav-active{/if}"{if $carousel_tabs == 'true'} id="ppbestsellers"{/if}>
    <div class="container">
        {if $carousel_tabs != 'true'}
            <p class="headline-section products-title">
              <a href="{$allbestProductsLink}" title="{$ppbestsellers_block_title|escape:'html':'UTF-8'}">
                {$ppbestsellers_block_title|escape:'html':'UTF-8'}
              </a>
            </p>
        {/if}
        {if $products}
            <div class="products grid row{if $carousel_active  == 'true'} view-carousel js-carousel-ppbest owl-carousel{else} view-grid xlarge-{$carousel_col} large-{$carousel_col_1200} medium-{$carousel_col_992} small-{$carousel_col_769} xsmall-{$carousel_col_576}{/if}"{if $carousel_active  == 'true'} data-carousel={$carousel_active} data-autoplay="{$carousel_autoplay}" data-speed="{$carousel_speed}" data-pag="{$carousel_pag}" data-arrows="{$carousel_arrows}" data-loop="{$carousel_loop}" data-rows="{$carousel_rows}" data-col="{$carousel_col}" data-col_1200="{$carousel_col_1200}" data-col_992="{$carousel_col_992}" data-col_769="{$carousel_col_769}" data-col_576="{$carousel_col_576}"{/if}>
