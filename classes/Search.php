...
class SearchCore
{
    ...
    public static function sanitize($string, $id_lang, $indexation = false, $iso_code = false, $keepHyphens = false)
    {
        ...
        $addtokey='';
        if (!empty($addtokey)) {
               $string = preg_replace('/(?<=\s)(' . $addtokey . ')(?=\s)/Su', 'golf4', $string);
        }
    ...
    public static function find(
        ...
        $addtokey='golf 4|golf 5';
        $addtokey='';
        if (!empty($addtokey)) {
            preg_match('/(?<=\s)(' . $addtokey . ')(?=\s)/Su',$expr,$matches);
            foreach ($matches as $matche) {
               $expr=str_replace($matche, str_replace(' ', '', $matche), $expr);
            }
            preg_match('/^(' . $addtokey . ')(?=\s)/Su',$expr,$matches);
             foreach ($matches as $matche) {
               $expr=str_replace($matche, str_replace(' ', '', $matche), $expr);
            }
            preg_match('/(?<=\s)(' . $addtokey . ')$/Su',$expr,$matches);
             foreach ($matches as $matche) {
               $expr=str_replace($matche, str_replace(' ', '', $matche), $expr);
            }
            preg_match('/^(' . $addtokey . ')$/Su',$expr,$matches);
             foreach ($matches as $matche) {
               $expr=str_replace($matche, str_replace(' ', '', $matche), $expr);
            }
            

            //$expr = preg_replace('/(?<=\s)(' . $addtokey . ')(?=\s)/Su', 'golf4', $expr);
            //$expr = preg_replace('/^(' . $addtokey . ')(?=\s)/Su', 'golf4', $expr);
            //$expr = preg_replace('/(?<=\s)(' . $addtokey . ')$/Su', 'golf4', $expr);
            //$expr = preg_replace('/^(' . $addtokey . ')$/Su', 'golf4', $expr);
        }
        $file=fopen("result_search.txt", "w+");
        fwrite($file, $expr);
        if (!$context) {
